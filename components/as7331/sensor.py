    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_UVA): sensor.sensor_schema(
                unit_of_measurement="counts", accuracy_decimals=0, state_class=STATE_CLASS_MEASUREMENT
            ),
            ...
            # optional irradiance outputs (scaling via multipliers)
            cv.Optional(CONF_UVA_IRR): sensor.sensor_schema(
                unit_of_measurement="µW/cm²", accuracy_decimals=3, state_class=STATE_CLASS_MEASUREMENT
            ),
...
async def to_code(config):
    ...
    if CONF_UVA_IRR in config:
        s = await sensor.new_sensor(config[CONF_UVA_IRR])
        cg.add(var.set_uva_irr_sensor(s))
    if CONF_UVB_IRR in config:
        s = await sensor.new_sensor(config[CONF_UVB_IRR])
        cg.add(var.set_uvb_irr_sensor(s))
    if CONF_UVC_IRR in config:
        s = await sensor.new_sensor(config[CONF_UVC_IRR])
        cg.add(var.set_uvc_irr_sensor(s))

    cg.add(var.set_uva_multiplier(float(config[CONF_UVA_MULT])))
    cg.add(var.set_uvb_multiplier(float(config[CONF_UVB_MULT])))
    cg.add(var.set_uvc_multiplier(float(config[CONF_UVC_MULT])))
