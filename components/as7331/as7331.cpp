...
/* TEMP block: TEMP + UVA + UVB + UVC (8 bytes) */
static constexpr uint8_t REG_TEMP  = 0x0A;

/* ================= OSR ================= */
static constexpr uint8_t OSR_CONFIG = 0x02;  // CONFIG
static constexpr uint8_t OSR_MEAS   = 0x03;  // MEAS
static constexpr uint8_t OSR_START  = 0x83;  // MEAS + START (CONT)

/* ================= Status ================= */
static constexpr uint8_t STATUS_NDATA = 0x08;

/* ================= Fixed measurement params =================
 * exakt wie im funktionierenden Stand
 */
static constexpr uint8_t GAIN     = 0;  // 0..11
static constexpr uint8_t INT_TIME = 7;  // 0..7

/* ================= LSB tables =================
 * (1:1 wie deine funktionierende Version)
 * unit: µW/cm² per count
 */
...
void AS7331Component::setup() {
  ...
  ESP_LOGI(TAG, "AS7331 started (CONT, TEMP-sync read)");
}
