...
void AS7331Component::setup() {
  ...
  ESP_LOGI(TAG, "AS7331 running (CONT mode)");
}
...
void AS7331Component::update() {
  ...
  float erythem = uvb_w + (uva_w * 0.002f);
  float uv_index = erythem / 0.025f;

  if (uv_index_) uv_index_->publish_state(uv_index);
}

void AS7331Component::dump_config() {
  ESP_LOGCONFIG(TAG, "AS7331 Spectral UV Sensor");
}
